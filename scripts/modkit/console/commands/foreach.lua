...
- $s: a modkit memgroup containing all the universe ships]],
    ...
    fn = function (param_vals, _, _, line)
        if (param_vals.lua) then
            ...
            parsed = gsub(parsed, "$s", 'modkit.table.map(hyperTableHandle()().foreach_src, function(ship_def) return register(ship_def.type, ship_def.player_id, ship_def.id, 1); end)');
